...
//Intro
Console.Clear();
int input = GetInput("The two of you enter a strange room. It's dark and difficult to see, but a dim light cracks through the "
...
input = 0;
    ...
    "Which way will you go?", "Left", "Right");
        ...
        stringInput = "";
        input = 0;
        int inputThree = GetInput("You head left, and quickly hit a right turn.\n", "Right", "Back");
        if (inputThree == 1)
        {
    ...
    inputFour = GetInputThreeOptions("You take the path ahead, finally ending at a fork in the road, to continue straight," +
            "or take a right?\n", "Forward", "Right", "Back");
    if (inputFour == 1)
        {
    ...
    int inputFive = GetInput("The path finishes with a large stretch of vines thick enough to climb on.\n" +
            "Climb up?", "Yes", "No");
        if (inputFive == 1)
        {
